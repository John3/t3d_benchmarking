...
{
	if(!isObject(%ragdollData))
	{
		error("createRagDoll: Unable to find dataBlock ["@%ragdollData@"]");
		return;
	}
	if(%ragdollData.getClassName() !$= "RagDollData")
	{
		warn("Trying to create RagDoll with NON RagDollData datablock: ["@%ragdollData@"]");
	}
